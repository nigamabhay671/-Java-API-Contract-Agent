...
if ($Mode -eq "Incremental") {
    ...
    if (Test-Path $srcPath) {
        ...
        $controllersText = if ($controllers.Count -eq 0) { 'None' } else { $controllers -join ', ' }
        $servicesText = if ($services.Count -eq 0) { 'None' } else { $services -join ', ' }
        $entitiesText = if ($entities.Count -eq 0) { 'None' } else { $entities -join ', ' }
        $repositoriesText = if ($repositories.Count -eq 0) { 'None' } else { $repositories -join ', ' }
